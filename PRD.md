...
5. **M4 — Observability**: (M4-A ✅) trace model + span tree (`Tracer`), pluggable `TraceStore` (in-memory default, lazy SQLite adapter), cost governor with budget caps, agent runtime records agent/model/tool spans and enforces budgets. (M4-B, next) `/_anvil` dashboard, OTel GenAI export.
