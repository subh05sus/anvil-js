...
6. **M5 — Durability + safety**: (M5-A ✅) pluggable `StateStore` (memory default, lazy SQLite), durable per-step checkpointing with side-effect fencing, HITL `meta.requestApproval` suspend + `resumeAgent` with injected approval. (M5-B, next) guardrails/policy middleware, prompt-injection taint layer.
