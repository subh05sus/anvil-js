...
7. **M6 — Evals + prompt registry + replay**: (M6-A ✅) `PromptRegistry` (versioned prompts, diff, pin, `renderPrompt`), evals harness (`anvil eval` — deterministic assertions: output contains/matches, tool-called, cost/iteration caps, schema-valid JSON; plus LLM-as-judge, itself traced/cost-tracked). (M6-B, next) `anvil replay <traceId>` re-running recorded steps with mocked model responses.
