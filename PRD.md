...
8. **M7 — Memory + retrieval**: (M7-A ✅) `MemoryStore` (`ctx.memory` get/set/append over the state store, namespaced per session) + RAG primitives — `Embedder`/`HashEmbedder`, `VectorStore`/`MemoryVectorStore`/lazy `SqliteVectorStore`, `chunkText`, `Retriever` with traced retrieval spans. (M7-B, next) `_context.ts` assembly pipeline + semantic caching.
