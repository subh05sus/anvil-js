...
9. **M8 — Protocol + background surface**: (M8-A ✅) scheduled agents (`cronMatches`, `Scheduler` with traced per-minute runs), event-triggered agents (`TriggerRegistry`), `loadBackgroundTasks` for `schedule.ts`/`trigger.ts`, and multi-agent orchestration (`AgentRegistry`, `agentAsTool` delegation, `withAgents`/`callAgent`). (M8-B, next) A2A protocol support, sandboxed `execute.ts` (post threat-model).
