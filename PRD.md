...
10. **M9 — Public launch** ✅: `docs/` (getting started, routing, MCP, agents, observability, durability & safety, evals/prompts/replay, memory/RAG, protocol/background), overhauled root README (pitch, feature matrix, quick start), 3 verified example apps — `basic-api` (Express-parity + MCP-exposed route + tool), `mcp-server` (zero-extra-code MCP quickstart), `agent-hitl` (suspend → approve → resume, live end-to-end). No separate marketing site — the README + docs are the launch surface for a framework repo.
